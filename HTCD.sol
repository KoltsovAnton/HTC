...
contract HTCDToken is Ownable {
    ...
    constructor (/*address _parentToken, uint _parentSnapShotBlock*/) public {
        name = "Hit The Crypto D";
        symbol = "HTCD";
        decimals = 18;
//        parentToken = HTCDToken(_parentToken);
//        parentSnapShotBlock = _parentSnapShotBlock == 0 ? block.number : _parentSnapShotBlock;
        parentToken = HTCDToken(address(0));
        parentSnapShotBlock = block.number;
        creationBlock = block.number;

        //initial emission
        uint _amount = 20000 * (10 ** uint256(decimals));
    ...
    function claimTokens(address _token) external onlyOwner {
        ...
        HTCDToken token = HTCDToken(_token);
